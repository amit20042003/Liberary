...
import {
    ...
    Phone, Image as ImageIcon, Settings, LogOut, CheckCircle, Edit, Trash2, Eye, History, BookMarked, Loader2, Printer, UploadCloud, ArrowLeft, XCircle, Mail, Lock, Download, FilterX, Sun, Moon, MessageSquare, UserX, UserCheck, KeyRound, EyeOff, TrendingUp
} from 'lucide-react';
...
const initialState = {
    ...
    // **MODIFIED**: Increased seat count to 100
    seats: Array.from({ length: 100 }, (_, i) => ({
        number: i + 1,
        // **MODIFIED**: Adjusted gender distribution for 100 seats
...
const App = () => {
    ...
    // **NEW**: State for total income visibility
    ...
    // **NEW**: Handler to verify password for income view
    ...
    // **NEW**: Calculation for total income
    const totalIncome = useMemo(() => {
        return students.reduce((total, student) => {
            const studentTotal = (student.payment_history || []).reduce((sum, payment) => sum + (payment.amount || 0), 0);
        ...
        <>
            ...
            <div className="flex h-screen bg-gray-100 font-sans">
                ...
                {isModalOpen && <ModalRouter
                    ...
                    onDepart={handleStudentDeparture}
                    onReactivateStudent={handleReactivateStudent}
                    libraryProfile={libraryProfile}
                    onWhatsApp={handleWhatsAppMessage}
                    onVerifyIncomePassword={handleVerifyIncomePassword}
                />}
            </div>
        </>
    );
...
const ModalRouter = ({ content, onClose, isSubmitting, ...props }) => {
    ...
    const renderModal = () => {
        switch (type) {
            ...
            // **NEW**: Case for the new income password modal
            case 'incomePassword': return <IncomePasswordModal onVerify={props.onVerifyIncomePassword} isSubmitting={isSubmitting} {...props} />;
            default: return null;
        }
    };
    return <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4"><div className={`bg-white p-6 md:p-8 rounded-lg shadow-2xl w-full ${type === 'printReceipt' || type === 'studentProfileDetail' || type === 'seatOccupantsDetail'
...
const Header = ({ userEmail, libraryName }) => (<header className="bg-white shadow-sm p-4 flex justify-between items-center"><h2 className="text-xl md:text-2xl font-semibold text-gray-700 truncate">{libraryName || 'Library Management'}</h2><div className="text-sm text-gray-600 hidden sm:block">{userEmail}</div></header>);
        ...
        {profile ? (
            ...
            <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <StatCard
                        label="Total Income"
                        value={isIncomeVisible ? `₹${totalIncome.toLocaleString('en-IN')}` : '••••••••'}
                        color="green"
                        icon={<TrendingUp />}
                        onClick={isIncomeVisible ? () => setIsIncomeVisible(false) : onIncomeClick}
                        isSensitive={true}
                        isRevealed={isIncomeVisible}
                    />
                </div>
                <div className="bg-white p-6 rounded-xl shadow-md">
...
const StatCard = ({ label, value, color, icon, onClick, isSensitive = false, isRevealed = false }) => {
    const colorClasses = {
        blue: { bg: 'bg-blue-100', text: 'text-blue-600', gradient: 'from-blue-500 to-blue-400' },
        red: { bg: 'bg-red-100', text: 'text-red-600', gradient: 'from-red-500 to-red-400' },
        yellow: { bg: 'bg-yellow-100', text: 'text-yellow-600', gradient: 'from-yellow-500 to-yellow-400' },
        green: { bg: 'bg-green-100', text: 'text-green-600', gradient: 'from-green-500 to-green-400' },
    };
    const { bg, text, gradient } = colorClasses[color] || colorClasses.blue;

    return (
        <div
            className={`relative p-5 rounded-xl shadow-md bg-white overflow-hidden group ${onClick ? 'cursor-pointer' : ''}`}
            onClick={onClick}
        >
            ...
            <div className="flex justify-between items-start">
                ...
                <div className={`p-3 rounded-full ${bg} ${text} group-hover:scale-110 transition-transform`}>
                    {icon}
                </div>
            </div>
            {isSensitive && (
        ...
        <div className="bg-white p-4 md:p-6 rounded-lg shadow-md">
            ...
            {searchResult ? (
                ...
                <>
                    ...
                    {/* **MODIFIED**: Responsive grid for seats */}
        ...
        <div className="space-y-6">
            <div className="bg-white p-4 md:p-6 rounded-lg shadow-md">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6 border-b pb-4">
                    ...
                    <Button onClick={resetFilters} className="bg-gray-200 text-gray-700 hover:bg-gray-300 w-full md:w-auto">
                        ...
                        <tbody>
                            {filteredStudents.length > 0 ? filteredStudents.map(s => {
                                ...
                                return (<tr key={s.id} className="border-b hover:bg-gray-50">
                                    ...
                                    <td className="p-3 hidden sm:table-cell"><span className={`px-3 py-1 rounded-full text-xs font-semibold ${s.status === 'departed' ? 'bg-gray-200 text-gray-700' : isDue ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>{s.status === 'departed' ? 'Inactive' : isDue ? 'Due' : 'Paid'}</span></td>
...
const DeparturesView = ({ departedStudents }) => (
    <div className="bg-white p-4 md:p-6 rounded-lg shadow-md">
        ...
        <div className="overflow-x-auto">
            <table className="w-full text-left">
                <thead><tr className="bg-gray-100"><th className="p-3">Name</th><th className="p-3 hidden sm:table-cell">Departure Date</th><th className="p-3 hidden md:table-cell">Reason</th><th className="p-3">Credit Transferred</th></tr></thead>
                    ...
                    {occupants.map(student => (
                        <div key={student.id} className="p-4 border rounded-lg flex flex-col sm:flex-row items-center gap-4 bg-gray-50">
                            ...
                            <div className="flex-1 text-center sm:text-left">
                                ...
                                <p className="text-sm font-mono text-indigo-600">Reg: {student.student_id}</p>
                                <div className={`inline-flex items-center gap-2 px-3 py-1 mt-2 rounded-full text-sm font-semibold ${student.admission_type === 'Full-time' ? 'bg-indigo-100 text-indigo-800' : student.shift === 'Morning' ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800'}`}>
                                    {student.admission_type === 'Full-time' ? <BookOpen size={16} /> : student.shift === 'Morning' ? <Sun size={16} /> : <Moon size={16} />}
                                    {student.admission_type === 'Full-time' ? 'Full-time' : `${student.shift} Shift`}
                                </div>
                            </div>
...
// **NEW**: Modal to enter password for viewing total income
const IncomePasswordModal = ({ onVerify, onClose, isSubmitting }) => {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <h3 className="text-2xl font-semibold mb-2 text-gray-800">Enter Password</h3>
            <p className="text-gray-600 mb-6">Please enter your account password to view the total income.</p>
            <div className="relative">
            ...
            <div className="flex justify-end gap-4 mt-6">
                <button type="button" onClick={onClose} disabled={isSubmitting} className="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 disabled:opacity-60">Cancel</button>
