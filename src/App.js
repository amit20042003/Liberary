...
import {
    ...
    Phone, Image as ImageIcon, Settings, LogOut, CheckCircle, Edit, Trash2, Eye, History, BookMarked, Loader2, Printer, UploadCloud, ArrowLeft, XCircle, Mail, Lock, Download, FilterX, Sun, Moon, MessageSquare, UserX, UserCheck, KeyRound
} from 'lucide-react';
...
const App = () => {
    ...
    useEffect(() => {
        ...
        const newSeats = initialState.seats.map(seat => ({
             ...seat,
             occupiedBy: { morning: null, evening: null }
        }));
    ...
    // **NEW**: Handler for viewing occupants of a single seat.
    const handleViewSeatOccupants = useCallback((occupantData) => {
        const { morningId, eveningId, seatNumber } = occupantData;
        const occupants = [];
        if (morningId) {
            const morningStudent = students.find(s => s.student_id === morningId);
            if(morningStudent) occupants.push(morningStudent);
        }
        if (eveningId && eveningId !== morningId) {
             const eveningStudent = students.find(s => s.student_id === eveningId);
            if(eveningStudent) occupants.push(eveningStudent);
        }
    ...
    return (
        <>
            <style>{`
                ...
                .styled-scrollbar::-webkit-scrollbar-track { background-color: #f0f0f0; border-radius: 10px; }
                .styled-scrollbar::-webkit-scrollbar-thumb { background-color: #c1c1c1; border-radius: 10px; border: 2px solid #f0f0f0; }
                .styled-scrollbar::-webkit-scrollbar-thumb:hover { background-color: #a8a8a8; }
            `}</style>
...
const LibrarySetupForm = ({ onSave, isSubmitting, onBack }) => {
    // Unchanged, so keeping it brief for clarity
...
const ModalRouter = ({ content, onClose, isSubmitting, ...props }) => {
    ...
    const renderModal = () => {
        switch (type) {
            ...
            // **NEW**: Case for the new modal to show multiple occupants
    ...
    return <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4"><div className={`bg-white p-6 md:p-8 rounded-lg shadow-2xl w-full ${
        type === 'printReceipt' || type === 'studentProfileDetail' || type === 'seatOccupantsDetail'
...
// **MODIFIED**: SeatMatrix now includes a search bar and new click handlers.
const SeatMatrix = ({ seats, activeStudents, onSeatClick, onViewStudent, onViewSeatOccupants }) => {
    ...
    const handleSeatSearch = () => {
        if (!searchQuery) {
            setSearchResult({ error: 'Please enter a seat number.' });
            return;
        }
        ...
        if (!seat) {
            setSearchResult({ error: `Seat #${seatNumber} does not exist.` });
            return;
        }
        ...
        if (morning) {
            const student = activeStudents.find(s => s.student_id === morning);
            if (student) occupants.push(student);
        }
        if (evening && evening !== morning) {
            const student = activeStudents.find(s => s.student_id === evening);
            if (student) occupants.push(student);
        }
    ...
    const getSeatState = (seat) => {
        ...
        if (morning) {
            return {
                style: { background: `linear-gradient(to bottom, #9ca3af 50%, ${emptyColor} 50%)` },
                className: 'border-gray-500 text-gray-800',
                icon: <Sun size={16} className="mt-1 text-yellow-600" />,
                isFull: false,
        ...
        if (evening) {
            return {
                style: { background: `linear-gradient(to top, #9ca3af 50%, ${emptyColor} 50%)` },
                className: 'border-gray-500 text-gray-800',
                icon: <Moon size={16} className="mt-1 text-blue-800" />,
                isFull: false,
                availableShift: 'Morning'
            };
        }
        return {
            className: `${baseColor} text-gray-700 hover:bg-green-300 hover:border-green-500`,
            icon: null,
            isFull: false
        };
    };
    ...
    const handleSeatClick = (seat, state) => {
        if (state.isFull) {
            onViewSeatOccupants({ morningId: seat.occupiedBy.morning, eveningId: seat.occupiedBy.evening, seatNumber: seat.number });
        } else if (state.availableShift) {
            onSeatClick({ number: seat.number, gender: seat.gender, prefillShift: state.availableShift });
        } else {
            onSeatClick({ number: seat.number, gender: seat.gender });
        }
    };
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
                ...
                <div className="flex gap-2 items-center w-full md:w-auto">
                    <div className="relative flex-grow">
                        <input
                            ...
                            onKeyDown={(e) => e.key === 'Enter' && handleSeatSearch()}
                            className="w-full p-2 pl-4 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        />
                    </div>
            ...
            {searchResult ? (
                ...
                <>
                    <div className="flex flex-wrap items-center space-x-4 mb-6 text-sm">
                        ...
                        {seats.map(seat => {
                            ...
                            return (
                                <div 
                                    key={seat.number} 
                                    style={state.style}
                                    className={`relative group w-full aspect-square flex flex-col items-center justify-center border rounded-md cursor-pointer transition-all duration-200 ${state.className}`} 
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="feeStatus" value={filters.feeStatus} onChange={handleFilterChange} className="w-full p-2 border border-gray-300 rounded-lg bg-white"><option value="all">All Fee Status</option><option value="paid">Paid</option><option value="due">Due</option></select>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Student Status</label>
                        <select name="studentStatus" value={filters.studentStatus} onChange={handleFilterChange} className="w-full p-2 border border-gray-300 rounded-lg bg-white"><option value="all">All Students</option><option value="active">Active</option><option value="departed">Inactive</option></select>
                    </div>
                </div>
            </div>
            <div className="bg-white p-6 rounded-lg shadow-md">
                ...
                <div className="overflow-auto max-h-[60vh] styled-scrollbar">
                    <table className="w-full text-left">
                        ...
                        <tbody>
                            {filteredStudents.length > 0 ? filteredStudents.map(s => {
                                const isDue = new Date(s.next_due_date) < getTodayDate(); const toDate = new Date(s.next_due_date); const fromDate = new Date(s.next_due_date); fromDate.setMonth(fromDate.getMonth() - 1);
                                return (<tr key={s.id} className="border-b hover:bg-gray-50">
                                        <td className="p-3"><p className="font-medium text-gray-800">{s.name} {s.status === 'departed' && <span className="text-xs text-red-500 font-normal">(Inactive)</span>}</p><p className="text-xs text-gray-500 font-mono">Reg: {s.student_id}</p></td>
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <div className="overflow-x-auto">
            <table className="w-full text-left">
                ...
                <tbody>{departedStudents.length > 0 ? departedStudents.map(s => (<tr key={s.id} className="border-b hover:bg-gray-50"><td className="p-3 font-medium">{s.name} (Reg: {s.student_id})</td><td className="p-3">{new Date(s.departure_date).toLocaleDateString()}</td><td className="p-3 text-sm text-gray-600">{s.departure_reason || <span className="text-gray-400">No reason given</span>}</td><td className="p-3">{s.transfer_log ? (<div><p className="font-medium">{s.transfer_log.transferredToName} <span className="font-normal text-green-700">({s.transfer_log.daysTransferred} days)</span></p><p className="text-xs text-gray-500 font-mono">Reg: {s.transfer_log.transferredToId}</p></div>) : (<span className="text-gray-500">N/A</span>)}</td></tr>)) : <tr><td colSpan="4" className="text-center p-4 text-gray-500">No students have departed yet.</td></tr>}</tbody>
            </table>
        </div>
    </div>
);
